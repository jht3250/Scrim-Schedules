...
import { Calendar, Clock, Plus, X, Trophy, Swords, Loader } from "lucide-react";
...
const TeamScheduler: React.FC<TeamSchedulerProps> = ({
    userId,
    viewingUserId,
    isViewOnly = false,
}) => {
    ...
    // Fetch teams and events on mount
    useEffect(() => {
        fetchTeams();
    ...
    const fetchTeams = async () => {
        try {
            ...
            // If viewing another user, fetch their teams
            if (viewingUserId) {
                // Get the user ID from username
                const { data: viewedProfile } = await supabase
                    .from("profiles")
                    .select("id")
                    .eq("username", viewingUserId)
                    .single();

                if (viewedProfile) {
                    query = query.eq("user_id", viewedProfile.id);
                }
            } else {
                // Otherwise fetch current user's teams
                query = query.eq("user_id", userId);
            }

            const { data: teamsData, error: teamsError } = await query.order(
                "name"
            );
                    ...
                    {teams.map((team) => (
                        ...
                        >
                            ...
                            <div className="space-y-2">
                                ...
                                {!isViewOnly && (
                                    <button
                                        onClick={() => openAddEventModal(team)}
